    ...
    const data = await page.evaluate(
      (postUrl, includePagesValue) => {
        ...
        const tags = tagCandidates
          ...
          .filter(
            (t) => t && t.length > 0 && t.length < 30 && !t.includes('\n'),
          )
          .filter((t) => isNaN(t))
          .filter((t) => !/^\d+\/\d+$/.test(t))
        ...
        const pagesPane = includePagesValue
          ? document.querySelector('#website-tabpane-pages')
          : null;
                ...
                const title =
                  rawTitle
                    ...
                    .find(
                      (part) =>
                        part &&
                        !/^(save|copy|show similar|more like this)$/i.test(
                          part,
                        ),
                    ) || rawTitle.trim();
        ...
        const sectionsBtn = Array.from(
          ...
          el.innerText?.trim().toLowerCase().startsWith('sections'),
        );
        ...
        const pagesBtn = includePagesValue
          ? document.querySelector('#website-tab-pages')
          : null;
